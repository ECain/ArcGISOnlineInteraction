 ...
 * 
 * Code demonstrates HttpWebRequests required for REST calls for interaction with ArcGIS.com organization accounts. Furthermore, 
 * it demonstrates the objects used to deserialize the JSON responses (HttpWebResponse) 
 * 
 ...
 * 
 * Code only supports Feature Services / Map Services interaction.
 * 
...
{
  static public class RequestAndResponseHandler
  {
    /// <summary>
    /// Returns a collection of Services within the organization (names only)
    /// </summary>
    ...
    /// <returns></returns>
    public static ServiceCatalog GetServiceCatalog(string url, string token, out string formattedRequest, out string jsonResponse)
    {
      string json = jsonResponse = Request(url, token, out formattedRequest);
    ...
    /// <summary>
    /// Returns a collection of the items contained within the signed in User's account. (My Content within the Web client)
    /// </summary>
    /// <param name="url"></param>
    /// <param name="token"></param>
    /// <param name="formattedRequest"></param>
    /// <param name="jsonResponse"></param>
    /// <returns></returns>
    public static UserOrganizationContent UserOrgContent(string url, string token, out string formattedRequest, out string jsonResponse)
    {
      string json = jsonResponse = Request(url, token, out formattedRequest);
      //Get the object based on the described DataContract
    ...
    /// <summary>
    /// Simple request to the server for basic information about the organization.
    /// </summary>
    /// <param name="url"></param>
    /// <param name="token"></param>
    /// <param name="request"></param>
    /// <returns></returns>
    private static string Request(string url, string token, out string request)
    {
    ...
    /// <summary>
    /// Return the thumbnail image of a service based on the url within the JSON response description for said service.
    /// </summary>
    /// <param name="url"></param>
    /// <returns></returns>
    ...
    /// <summary>
    /// Return complete Service attribution for a named feature service. Service Catalog calls provide the 
    /// basics of an organizations content. Using the name of a particular service, request detailed
    /// infomation for that particular service.
    /// </summary>
    ...
    /// <param name="jsonResponse"></param>
    /// <returns></returns>
    public static FeatureLayerAttributes GetFeatureServiceAttributes(string baseURL, string token, out string formattedRequest, out string jsonResponse)
    {
      //get attributes for the layer
      formattedRequest = baseURL + "?f=pjson&token=" + token;
      HttpWebResponse httpResponse = HttpWebGetRequest(formattedRequest, "");

      //get the JSON representation from the response
      string json = jsonResponse = DeserializeResponse(httpResponse.GetResponseStream());
    ...
    /// <summary>
    /// Self calls return details about the user signing in, full name, email address, current role within the organization,
    /// data usage, data quota, organization unique identifier
    /// </summary>
    /// <param name="url"></param>
    /// <param name="token"></param>
    /// <param name="formattedRequest"></param>
    ...
    /// <returns></returns>
    public static Self SelfWebRequest(string url, string token, out string formattedRequest, out string responseJSON)
    {
      ...
      try
      {
        formattedRequest = url + "?f=pjson&token=" + token;
        httpResponse = HttpWebGetRequest(formattedRequest, "");
      }
      catch { return null; }
    ...
    //GET HttpWebRequests are request with payload less than 2000 chars used for not overly complex payloads.
    //For complex payloads one must use a POST.
    ...
    public static FeatureEditsResponse FeatureEditRequest(string baseURL, string jsonEdits, out string jsonResponse)
    {
      ////get the JSON representation from the response
      string json = jsonResponse = GetJSONResponseString(baseURL, jsonEdits);

      //Get the object based on the described DataContract
      return GetObjectFromJSON(json, DataContractsEnum.FeatureEditsResponse) as FeatureEditsResponse;
    }

    public static FeatureQueryResponse FeatureQueryRequest(string queryString, out string jsonResponse)
    {
      //make the request to the server
    ...
    public static string HttpWebRequestHelper(string url, string transmissionContent, string referer, out string formattedRequest)
      {
        formattedRequest = "POST: URL=" + url + ", Content: " + transmissionContent + ", Referer: " + referer;
    ...
    public static Authentication AuthorizeAgainstArcGISOnline(string username, string password, string referer, out string formattedRequest, out string jsonResponse)
    {
      ...
      try
      {
        JSON = HttpWebRequestHelper(url, jsonTransmission, referer, out formattedRequest);
        jsonResponse = JSON;
      }
      catch { return null; }
    ...
    public static bool IsFeatureServiceNameAvailable(string featureServiceName, string serviceURL,  string token, string referer, out string formattedRequest, out string jsonResponse)
    {
      if (!serviceURL.EndsWith("/"))
        serviceURL += "/";

      //build the GET request
      string url = formattedRequest = serviceURL + "isServiceNameAvailable?name=" + featureServiceName + "&type=Feature_Service&f=json&token=" + token;

      HttpWebResponse response = HttpWebGetRequest(url, referer);
      ...
      //get the JSON representation from the response
      string json = jsonResponse = DeserializeResponse(response.GetResponseStream());
    ...
    public static FeatureServiceCreationResponse CreateNewFeatureService(string featureServiceName, string serviceURL, string token, string referer, out string formattedRequest, out string jsonResponse)
    {
      formattedRequest = string.Empty;
      jsonResponse = string.Empty;
      ...
      createFeatServObjParams.xssPreventionInfo = new XssPreventionInfo()
      {
        ...
        xssPreventionRule = "InputOnly", //TODO: create an enum representation of values            
        xssInputRule = "rejectInvalid"
      };//TODO: create an enum representation of values  

      //createFeatServObjParams.spatialReference = new SpatialReference(){
      //  wkid = 102100,
      //  latestWkid = 3857
      //};
      //Extent ext = new Extent(){
      // xmin = -14999999.9999997, 
      //  ymin = 1859754.53234478, 
      //  xmax = -6199999.9999999, 
      //  ymax = 7841397.32770119, 
      //  spatialReference =  createFeatServObjParams.spatialReference
      //};

      //createFeatServObjParams.initialExtent = ext;
      //createFeatServObjParams.fullExtent = ext;

      //createFeatServObjParams.layers = new Layer[1]{new Layer(){
      //  id = 0,
      ...
      string JSON = jsonResponse =HttpWebRequestHelper(serviceURL, featureServiceInfoString, referer, out formattedRequest);
    ...
    public static bool ShareFeatureService(string itemID, string url, bool everyone, string groups, bool account, string token, string referer, out string formattedRequest, out string responseJSON)
    {
      ...
      string JSON = HttpWebRequestHelper(url, sharing, referer, out formattedRequest);
      responseJSON = JSON;
